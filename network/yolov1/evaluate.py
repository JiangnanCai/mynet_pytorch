...
def evaluate(preds, targets, class_names, threshold=0.5):
    ...
    for class_name in class_names:
        ...
        prob_preds = [pred[1] for pred in class_preds]
        boxes_preds = [pred[2:] for pred in class_preds]

        # Sort lists by probs.
        sorted_idxs = np.argsort(prob_preds)[::-1]
        image_fnames = [image_fnames[i] for i in sorted_idxs]
        boxes_preds = [boxes_preds[i] for i in sorted_idxs]
        ...
        num_detections = len(boxes_preds)
        ...
        for det_idx, (filename, box_pred) in enumerate(zip(image_fnames, boxes_preds)):  # 预测值

            if (filename, class_name) in targets:
                boxes_gt = targets[(filename, class_name)]
                for box_gt in boxes_gt:  # 两个for循环匹配box
                    # print(box_gt, box_pred)
                    # Compute IoU b/w/ predicted and ground-truth boxes.
                    inter_x1 = max(box_gt[0], box_pred[0])
                    inter_y1 = max(box_gt[1], box_pred[1])
                    inter_x2 = min(box_gt[2], box_pred[2])
                    inter_y2 = min(box_gt[3], box_pred[3])
                    inter_w = max(0.0, inter_x2 - inter_x1 + 1.0)
                    inter_h = max(0.0, inter_y2 - inter_y1 + 1.0)
                    inter = inter_w * inter_h

                    area_det = (box_pred[2] - box_pred[0] + 1.0) * (box_pred[3] - box_pred[1] + 1.0)
...
def map_calculate():
    ...
    model_path = '/home/cai/Documents/mynet_pytorch/network/yolov1/model_latest.pth'
    # GPU device on which yolo is loaded.
    gpu_id = 0
    # Path to label file.
    label_path = '/home/cai/Documents/mynet_pytorch/DataSet/labels/val'
    # Path to image dir.
    image_dir = '/home/cai/Documents/mynet_pytorch/DataSet/images/val'

    annotations = [os.path.join(label_path, path) for path in os.listdir(label_path)]
    ...
    for anno in annotations:
        filename = anno.split("/")[-1].split(".")[0] + ".jpg"
        image_fnames.append(filename)

        img = cv2.imread(os.path.join(image_dir, filename))
        h, w = img.shape[:2]

        with open(anno, 'r') as r:
            lines = r.readlines()
        lines = np.array([list(map(float, line.strip('\n').split())) for line in lines])

        for i in lines:
            x1 = int((i[1] - 1 / 2 * i[3]) * w)
            y1 = int((i[2] - 1 / 2 * i[4]) * h)
            x2 = int((i[1] + 1 / 2 * i[3]) * w)
            y2 = int((i[2] + 1 / 2 * i[4]) * h)
            class_name = voc_class_names[int(i[0]) - 1]
    ...
    return evaluate(preds, targets, class_names=voc_class_names)
